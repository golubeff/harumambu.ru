module ResourceThis # :nodoc:
  ...
  module ClassMethods
    def resource_this(options = {})
      options.assert_valid_keys(:class_name, :will_paginate, :finder_options, :nested, :path_prefix, :redirect_to_index, :includes, :resource_url, :collection_url)
      ...
      module_eval <<-"end_eval", __FILE__, __LINE__
        def load_#{singular_name}
          @#{singular_name} = finder_base.find(params[:id], :include => #{includes})
        end
        ...
        def create_#{singular_name}
          returning true do
            @#{singular_name} = finder_base.new(params[:#{controller_name.singularize}])
            @#{singular_name}.user = @current_user if @current_user && @#{singular_name}.respond_to?(:user=)
        ...
        def update_#{singular_name}
          returning true do
            @#{singular_name}.user = @current_user if @current_user && @#{singular_name}.respond_to?(:user=)
            @updated = @#{singular_name}.update_attributes(params[:#{controller_name.singularize}])
          end
        end
      ...
      if will_paginate_index
        module_eval <<-"end_eval", __FILE__, __LINE__
          def load_#{plural_name}
            @#{plural_name} = finder_base.paginate(finder_options.merge(:page => params[:page], :per_page => #{will_paginate_index}))
          end
        end_eval
      else
        module_eval <<-"end_eval", __FILE__, __LINE__
        ...
        def create
          respond_to do |format|
            if @created
              ...
              format.html { redirect_to(#{redirect_to_index} ? collection_url : resource_url) }
        ...
        def destroy          
          respond_to do |format|
            if @destroyed
              ...
              flash[:alert] = "Запись не может быть удалена."
              format.html { render :action => :edit }
            end
          end
        end
